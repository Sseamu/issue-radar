...
**하루 일정 (기본값: `BRIEF_AT` 비움 = 준비되는 즉시 전송)**
```
05:55  BIOS 자동 부팅 → 05:57 작업 시작 (Ollama·모델 로딩 약 1~2분)
06:00  채점 반영 → 관심 키워드 (2일 동향)
06:05  아침 브리핑 생성 → 바로 Slack 전송 (06시 10분 전후 도착)
이후   밤사이 쌓인 @radar 요청 처리 (1건당 약 2분) → 끝나면 PC 종료 (2분 예고)
```
- 알림 시각을 고정하고 싶으면 `.env`에 `BRIEF_AT=07:00`을 적으세요. `BRIEF_SCHEDULE=1`(기본)이면 브리핑을 미리 만들어 Slack **예약 전송**을 걸고 PC는 먼저 꺼집니다. `0`이면 07:00까지 켜 두고 직전 뉴스로 만들어 보냅니다.
- **처리 용량**: 06:00~06:55 사이 약 **20건** (1건당 실측 약 2분, 예산 계산은 3분으로 여유 있게). 넘치면 스레드에 "내일 처리" 안내를 남깁니다.
- 요청이 적은 날은 06시 10~20분쯤 모든 일을 마치고 꺼집니다. 브리핑 내용은 생성 시점까지의 뉴스 기준입니다.
